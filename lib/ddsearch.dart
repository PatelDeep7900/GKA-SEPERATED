...
import 'models/csc/CountryModel.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _statefromcountry(String country_id)async {
    ...
    try {
      var url = "http://192.168.10.141:8084/GKARESTAPI/c_cscpicker?cond=state&country_id=${country_id}";
  ...
  Future<void> _cityfromstate(String state_id)async {
    try {
      var url = "http://192.168.10.141:8084/GKARESTAPI/c_cscpicker?cond=city&state_id=${state_id}";
      print(url);
      var uri = Uri.parse(url);
      print(uri);
      final response = await http.get(uri);
                ...
                onChanged: (value) {
                  setState(() {
                    _countryval = value;
                     ...
                     statelist=[{"name":"SELECT","id":"0"}];
                     citylist=[{"name":"SELECT","id":"0"}];
