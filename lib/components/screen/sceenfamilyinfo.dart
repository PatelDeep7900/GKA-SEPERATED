...
import 'package:gka/models/familyinfo/familyinfo.dart';
...
class _HomePageState extends State<familyinfoscreen> {
  ...
  Future<void> getfamilyinfoall() async{
    ...
    try {
      ...
      if (response.statusCode == 200) {
        setState(() {
          List<familyinfo> welcome = familyinfo.fromJsonList(json.decode(response.body));
          _items=welcome;
        });
      }
      ...
      final response = await http.post(
        ...
        var data=json.decode(response.body);
          ...
          if(result==true){
            ...
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Insert Data Successfully...')));// Close th
          }else{
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if(result==true){
          ...
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Update Data Successfully...')));
          ...
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error While Updating Data')));
          print("not saved data");
        }
  ...
  void _showForm(BuildContext ctx, int? itemKey) async {
    ...
    showModalBottomSheet(
        ...
        shape: OutlineInputBorder(borderSide: BorderSide(color: Colors.white),borderRadius: BorderRadius.only(topLeft: Radius.circular(15),topRight:Radius.circular(15))),
        context: ctx,
        elevation: 5,
                  ...
                  children: [
                    Expanded(
                      child: TextFormField(
                        ...
                        decoration: const InputDecoration(hintText: 'self/Spouse/Child'),
                        validator: (val){
                          if(val!.isEmpty){
                    ...
                    Expanded(
                      child: TextFormField(
                        controller: _dobController, //editing controller of this TextField
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Family Information',style: TextStyle(color: Colors.black)),
        centerTitle: true,
        actions: [
          Padding(
            padding: const EdgeInsets.all(8.0),
            child: InkWell(child: const Chip(label: Row(children: [Text('add',style: TextStyle(color: Colors.white),),Icon(Icons.add)],),backgroundColor: Colors.green,),onTap: () {
